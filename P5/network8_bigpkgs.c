...
int receive_first(int* arg)
{
    ...
    for (i=0; i<MAX_COUNT; i++) {
        ...
        sprintf(sendbuffer, "Received packet %d.\n", i+1);
        sendbuffer[strlen(sendbuffer) + 1] = '_';
        if (minimsg_send(port, from, sendbuffer, MINIMSG_MAX_MSG_SIZE) == -1)
...
int transmit_first(int* arg)
{
    ...
    sendbuffer[MINIMSG_MAX_MSG_SIZE - 1] = '_';
    sendbuffer[MINIMSG_MAX_MSG_SIZE] = '\0';
    if (minimsg_send(port, dest, sendbuffer, MINIMSG_MAX_MSG_SIZE + 1) != -1)
        printf("Able to send messages longer than MINIMSG_MAX_MSG_SIZE.\n");
    else
        minimsg_send(port, dest, sendbuffer, MINIMSG_MAX_MSG_SIZE);
